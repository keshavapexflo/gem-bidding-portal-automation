...
def dense_confidence(distance: float) -> float:
    ...
    Values are calibrated against the normalized BGE collection built by the
    initial embedding pipeline. Strong matches in that collection are commonly
    around 0.25--0.55 L2 distance, while weak matches approach or exceed 1.0.
    """
    if distance <= 0.50:
        return 1.0
    if distance <= 0.70:
...
class HybridRetriever:
    def __init__(
        ...
        *,
        ...
        query_model_revision: str = BGE_MODEL_REVISION,
    ):
        ...
        self.query_model_revision = query_model_revision
    ...
    def search(
        ...
        *,
        ...
        lexical = (
            ...
            if sidecar_is_ready
            else self._native_lexical_search(query, lexical_candidates or candidates, effective_where)
        )
        if exclude_boilerplate:
