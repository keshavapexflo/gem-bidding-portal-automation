...
def download_single_document(session, bid, total_tasks, destination_dir):
    ...
    try:
        # requests.Session is not guaranteed to be thread-safe. Give each
        # worker its own session while copying the authenticated cookies.
        worker_session = requests.Session()
        worker_session.headers.update(session.headers)
        worker_session.cookies.update(session.cookies)
        resp = worker_session.get(url, timeout=60, stream=True)
        resp.raise_for_status()
        ...
        temporary_path = f"{filepath}.part"
        ...
        with open(temporary_path, "rb") as downloaded_file:
            signature = downloaded_file.read(5)
        if signature != b"%PDF-":
            content_type = resp.headers.get("Content-Type", "unknown")
            raise ValueError(
                f"GeM returned a non-PDF response (Content-Type: {content_type})"
            )
    ...
    while (
        ...
        for p in range(page_num, page_num + pages_to_fetch):
            try:
                ...
                raise RuntimeError(
                    f"Aborting sync because GeM listing page {p} could not be fetched safely"
                ) from e
        ...
        unique_candidates = {bid["bid_id"]: bid for bid in candidates}
        to_download = [
            bid for bid in unique_candidates.values()
            if bid["bid_id"] not in downloaded_bids
        ]
        if MAX_NEW_DOWNLOADS_PER_RUN:
