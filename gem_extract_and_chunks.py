...
def extract_and_chunk_pdf(pdf_path, manifest_entry=None, pdf_path_value=None):
    ...
    for i, c in enumerate(raw_chunks):
        ...
        if c['chunk_type'] == "atc_clause":
            # Clause numbers can repeat with different text inside one GeM PDF.
            # A text fingerprint prevents mismatched clauses from colliding,
            # while identical rendering duplicates collapse to one chunk.
            normalised_text = re.sub(r"\s+", " ", c["text"]).strip().lower()
            text_fingerprint = hashlib.sha256(normalised_text.encode("utf-8")).hexdigest()[:12]
            chunk_id += f"::{c.get('clause_no', i)}::{text_fingerprint}"
...
def process_directory(
    ...
    prev_state = load_existing_chunks_state(state_path or output_path)
    ...
    for idx, pdf_path in enumerate(files):
        ...
        if not force_reprocess and prev and prev.get("metadata", {}).get("document_hash") == current_hash:
    ...
    if sync_path:
        ...
        os.makedirs(os.path.dirname(os.path.abspath(sync_path)), exist_ok=True)
        temporary_sync = f"{sync_path}.tmp"
        with open(temporary_sync, "w", encoding="utf-8") as f:
            json.dump(sync_data, f, indent=2)
        os.replace(temporary_sync, sync_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bids-dir", default=os.path.join(PROJECT_DIR, "downloads", "bids"))
    parser.add_argument("--manifest", default=os.path.join(PROJECT_DIR, "downloads", "downloaded_bid_manifest.json"),
    ...
    parser.add_argument("--output", default=os.path.join(PROJECT_DIR, "bid_chunks.json"))
    parser.add_argument("--sync-output", default=os.path.join(PROJECT_DIR, "downloads", "chunk_sync.json"),
